...
def _score_bounds(calib_records: list[DocumentRecord], test_records: list[DocumentRecord]) -> tuple[float, float]:
    values: list[float] = []
    for record in list(calib_records) + list(test_records):
        values.extend(float(candidate.score) for candidate in record.candidates)
        values.append(float(record.success_score))
    if not values:
        return 0.0, 1.0
...
def _predict_thresholds(
    ...
    for idx, value in enumerate(unique_vals):
        threshold = condconf.predict(
            ...
            S_min=float(score_min),
            S_max=float(score_max),
            exact=exact,
        )
        thresholds[idx] = float(np.asarray(threshold).reshape(-1)[0])
    return np.clip(thresholds[inverse], float(score_min), float(score_max))


def _predict_thresholds_naive(
    condconf: CondConf,
    difficulty: np.ndarray,
    alpha: float,
    score_min: float,
    ...
    return np.clip(np.asarray(thresholds, dtype=float).reshape(-1), float(score_min), float(score_max))
...
def run_learnt_cp(
    ...
    score_min, score_max = _score_bounds(calib_records, test_records)
    thresholds = _predict_thresholds_naive(
        condconf,
        test_difficulty,
        alpha=alpha,
        score_min=score_min,
...
def run_cfc_full(
    ...
    test_difficulty = difficulty_array(test_records, proxy=proxy)
    score_min, score_max = _score_bounds(calib_records, test_records)
    thresholds = _predict_thresholds(
        condconf,
        test_difficulty,
        alpha=alpha,
        exact=exact,
