...
def split_conformal_quantile(scores, quantile):
    scores = np.sort(np.asarray(scores, dtype=float).reshape(-1))
    if len(scores) == 0:
        raise ValueError("scores must be non-empty.")

    rank = int(np.ceil((len(scores) + 1) * float(quantile)))
...
def fit_model(data_train, base_model):
    ...
    if base_model == "ols":
        ...
        reg.predict = lambda x, q: (
            np.asarray(x).reshape(1, -1) if np.asarray(x).ndim == 1 else np.asarray(x)
        ) @ reg._get_calibration_solution(q)[1]
        reg.predict = lambda x, q, _predict=reg.predict: np.asarray(_predict(x, q)).reshape(-1)
    return reg
...
def get_coverage(dataset_calib, dataset_test, score_fn, method, quantile):
    if method == "split":
        ...
        score_cutoff = split_conformal_quantile(scores_calib, quantile)
        if quantile >= 0.5:
...
def get_cutoff(dataset_calib, dataset_test, score_fn, method, quantile):
    ...
    if method == "split":
        scores_calib = score_fn(*dataset_calib)
        score_cutoff = split_conformal_quantile(scores_calib, quantile)
        cutoffs = np.full((len(scores_test),), score_cutoff)
    elif "rand" in method:
        condcalib = CondConf(score_fn, lambda x: x)
        condcalib.setup_problem(*dataset_calib)
        cutoffs = []
